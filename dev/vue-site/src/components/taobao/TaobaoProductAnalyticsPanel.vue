...
import { fetchTaobaoProductAnalytics } from '@/api/taobaoApi'
import { BESTSELLER_TIERS, classifyBestsellerTier, formatAnalyticsAmount } from '@/utils/taobaoAnalytics'
...
async function load() {
  ...
  loading.value = true
  try {
    const data = await fetchTaobaoProductAnalytics({ type: props.type, storeId: props.selectedStoreId })
