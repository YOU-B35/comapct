...
import {
  fetchTaobaoOrderOverview,
  fetchTaobaoOrderSummary,
  fetchTaobaoOrderTrend,
} from '@/api/taobaoApi'
...
async function load() {
  ...
  try {
    ...
    if (props.selectedStoreId === 'all') {
      overview.value = await fetchTaobaoOrderOverview({ startDate: start, endDate: end })
    } else {
      overview.value = null
    }
    const [summaryRes, trendRes] = await Promise.all([
      fetchTaobaoOrderSummary({ startDate: start, endDate: end, storeId: props.selectedStoreId }),
      fetchTaobaoOrderTrend({ startDate: start, endDate: end, storeId: props.selectedStoreId }),
    ])
    ...
    trend.value = []
    overview.value = null
    ElMessage.error(error?.message || '加载淘宝经营驾驶舱失败')
  } finally {
    loading.value = false
  }
}
