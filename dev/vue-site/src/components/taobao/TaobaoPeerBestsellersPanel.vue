...
async function load() {
  ...
  loading.value = true
  try {
    const data = await fetchTaobaoPeerBestsellers({
...
<template>
  <div class="taobao-peer">
    <div class="toolbar">
      <el-tooltip placement="top" :content="'销量为平台公开「已售 X 件」展示口径，平台未标注为累计或近30天，以淘宝商品页展示为准'">
