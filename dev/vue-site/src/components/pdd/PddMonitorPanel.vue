...
async function loadTargets() {
  ...
  try {
    const data = await listPddMonitorTargets()
    ...
    ElMessage.error(error?.message || '加载拼多多竞店监控失败')
  }
}
...
async function loadTrend() {
  trend.value = await fetchPddMonitorTrend(selectedTargetId.value, {
    days: 30,
...
function renderTrend() {
  const chartEl = document.getElementById('pdd-monitor-trend')
  if (!chartEl) return
  if (trendChart) {
    trendChart.dispose()
    trendChart = null
...
async function trigger(targetId) {
  try {
    ...
    while (Date.now() < deadline) {
      ...
      const job = await fetchPddMonitorJob(jobId)
...
async function saveTarget() {
  ...
  try {
    const created = await createPddMonitorTarget(payload)
    if (created?.id) {
      await updatePddMonitorSchedule(created.id, {
        enabled: true,
...
async function removeTarget(targetId) {
  try {
    ...
    await deletePddMonitorTarget(targetId)
...
<template>
  <div class="pdd-monitor">
      ...
      <el-form label-width="110px">
        ...
        <el-form-item label="监控类型">
          <el-select v-model="form.crawl_strategy">
            <el-option label="店铺爆款 Top N" value="pdd_shop_topn" />
    ...
    <el-card shadow="never" style="margin-top: 12px">
      ...
      <div id="pdd-monitor-trend" style="height: 320px"></div>
    </el-card>
