...
import { fetchPddProductAnalytics } from '@/api/pddApi'
import { BESTSELLER_TIERS, classifyBestsellerTier, formatAnalyticsAmount } from '@/utils/pddAnalytics'
...
async function load() {
  ...
  loading.value = true
  try {
    const data = await fetchPddProductAnalytics({ type: props.type, storeId: props.selectedStoreId })
