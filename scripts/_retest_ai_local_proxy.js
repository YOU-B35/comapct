...
print('local_proxy_http=', code, 'sec=', sec, 'bytes=', len(raw), flush=True)
...
with open('/opt/1panel/www/sites/www.yoto.work/log/access.log','r',errors='ignore') as f:
  for line in f:
    ...
    if '13/Aug/2026:16' not in line and '13/Aug/2026:15' not in line: continue
    parts=line.split()
    if len(parts)>8: cnt[parts[8]] += 1
print('access_15_16=', dict(cnt), flush=True)
# last 8 lines
print('--- last requests ---', flush=True)
os.system("grep 'api-proxy/images' /opt/1panel/www/sites/www.yoto.work/log/access.log | tail -8 | awk '{print $4,$7,$9,$10}'")
PY`;
  c.exec(cmd,(e,stream)=>{
