...
def build_amazon_system_prompt(store_name: str) -> str:
    return (
        ...
        "8. 页面 URL 地图：账户健康/绩效=https://sellercentral.amazon.com/performance/dashboard"
        "（新卖家平台的账户状况与配送绩效指标在 Dashboard 上；"
        "若该页读不到订单缺陷率/迟发率，再试 https://sellercentral.amazon.com/performance/account/health）；"
        "订单=https://sellercentral.amazon.com/orders-v3/?page=1；"
        "销售额/收入/商品报告=https://sellercentral.amazon.com/business-reports/detail/sales-traffic-by-asin"
        "（销售快照含「今天到目前为止」的已订购商品销售额与数量，并对比昨日/上周同日）；"
        "新平台首页今日销售额=https://sellercentral.amazon.com/amazonsell/business；"
